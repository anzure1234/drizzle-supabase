...
import { addDays, format, startOfWeek, eachDayOfInterval, isSameDay, isAfter, set, isBefore, isWeekend } from "date-fns"
...
export default function Component() {
    ...
    const daysToDisplay = eachDayOfInterval({ start: currentWeekStart, end: addDays(nextWeekStart, 6) })

    const isOrderable = (date: Date) => {
        const orderCutoffTime = set(addDays(date, -1), { hours: 21, minutes: 0, seconds: 0 })
        return isAfter(date, currentTime) && !isWeekend(date) && isBefore(currentTime, orderCutoffTime)
    }
    ...
    return (
        <div className="container mx-auto p-4">
            <header className="text-center mb-8">
                ...
                <p className="text-xl">Current Week: {format(currentWeekStart, "MMMM d")} - {format(addDays(currentWeekStart, 6), "MMMM d, yyyy")}</p>
                <p className="text-xl">Next Week: {format(nextWeekStart, "MMMM d")} - {format(addDays(nextWeekStart, 6), "MMMM d, yyyy")}</p>
            </header>
                        ...
                        <div key={index} className="relative">
                            <Button
                                ...
                                className={`h-24 w-full flex flex-col items-center justify-center ${
                                    ...
                                    isOrdered ? "bg-green-500 hover:bg-green-600" : ""
            ...
            <Dialog open={isMealDialogOpen} onOpenChange={setIsMealDialogOpen}>
                <DialogContent className="sm:max-w-[425px]">
                    <DialogHeader>
                        <DialogTitle>Meals for {selectedDate && format(selectedDate, "EEEE, MMMM d")}</DialogTitle>
                        <DialogDescription>Select your meal for this day</DialogDescription>
                    </DialogHeader>
                    <div className="grid gap-4 py-4">
                        {meals.map((meal) => (
                            <Card key={meal.id}>
                                ...
                                <CardFooter>
                                    <Button onClick={() => handleOrderClick(meal)}>Order this meal</Button>
                                </CardFooter>
                            </Card>
                        ))}
                    </div>
            ...
            <AlertDialog open={isCancelDialogOpen} onOpenChange={setIsCancelDialogOpen}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            Are you sure you want to cancel your order for {selectedDate && format(selectedDate, "MMMM d, yyyy")}?
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
            ...
            <div className="mt-8">
                ...
                {Array.from(orders).map(([dateString, meal]) => (
                    <div key={dateString} className="mb-2">
                        <span className="font-semibold">{format(new Date(dateString), "EEEE, MMMM d")}:</span> {meal.name}
                    </div>
                ))}
            </div>
        </div>
